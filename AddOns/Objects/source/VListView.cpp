...
: VObject()
{
	...
	// -si c'est un objet Be (BListView,BButton etc ...)
	...
	_properties->ReplaceString(VOBJECT_CLASS,"BListView");
	...
	_infos->AddString(VOBJECT_ABOUT,"BListView is the Be ListView object\nIt's the simple list View from BeOS API.");
...
status_t VListView::CreateObject(BMessage *archive)
{
	// on part du principe que le message est correcte
	// pas de probleme on creer l'objet
	_object = BListView::Instantiate(archive);
	if(_object==NULL)
		return B_ERROR;
...
status_t VListView::SetDefaultProperties(BMessage *properties)
{
	status_t	state;
...
status_t VListView::SetProperties(BMessage *value,int32 index)
{
	if(value==NULL)
		return B_ERROR;
	...
	if(LooperLock())
	{
		...
		if(value->FindString("_name",&name)==B_OK)
			((BListView *)_object)->SetName(name.String());
		...
		printf("Erreur : LooperLock() dans VListView::SetProperties()\n");
